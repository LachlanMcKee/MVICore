...
class BaseFeatureKtxTest {
    ...
    private fun testFeature(func: suspend TurbineContext.(ReceiveTurbine<TestState>) -> Unit) =
        runTest {
    ...
    fun `emitted initial state is correct`() =
        testFeature { turbine ->
            assertEquals(TestState(), turbine.awaitItem())
        }

    @Test
    fun `there should be no state emission besides the initial one for unfulfillable wishes`() =
        testFeature { turbine ->
            feature.invoke(TestWish.Unfulfillable)
            feature.invoke(TestWish.Unfulfillable)
            feature.invoke(TestWish.Unfulfillable)

            assertEquals(TestState(), turbine.expectMostRecentItem())
        }
    ...
    fun `there should be the same amount of states as wishes that translate 1 - 1 to effects plus one for initial state`() =
        testFeature { turbine ->
            ...
            turbine.skipItems(1)
            assertEquals(TestState(counter = 102), turbine.awaitItem())
            assertEquals(TestState(counter = 104), turbine.awaitItem())
            assertEquals(TestState(counter = 106), turbine.awaitItem())
        }
